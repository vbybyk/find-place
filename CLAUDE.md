...
Detail columns: `rooms_number` (bedrooms), `bathrooms`, `parking`, `area_total` (floor area m²), `furnished`, `discount` (percent off `price`), and `amenities` (string[] of keys — see `Amenities` in constants).

These id→label maps live once in **`src/constants/listings.ts`** (`ListingTypes`, `PropertyTypes`, `Countries`, `FurnishedTypes`, `Amenities`) — import them, never re-declare inline. The listings page filters via **query params** (`type`/`houseType`/`city`/`userId`) passed to `getListings`, which maps them to the API's `type`/`house_type`/`city`/`user_id` filters.

### Forms

Two flows, one shared set of field **sections** in `features/listings/forms/sections/` (`CategorySection`, `LocationSection`, `BasicsSection`, `DetailsSection`, `PriceSection`, `PhotosSection`), each taking `control` (+ `setValue`/`watch` where needed):
- **Edit** → `CreateListingForm.tsx` — the single-page form (all sections in one `<form>`); hydrates from the flat `IListing` via `setValue` in a `useEffect` and is reached at `/listings/[id]?userId=…`.
- **Create** → `CreateListingWizard.tsx` (mounted at `/listings/new`) — a 5-thing Airbnb-style wizard: **Type → Location → Basics → Details → Photos → Price → Review** (7 screens), progress bar, Back/Next with per-step required checks, `Publish` on Review.

Both share the **internal** form shape (`IListingFormValues`), which keeps location under a nested `location` object with **flat** Google-derived fields (`placeId`, `city`, `admin1`, `barangay`, `postalCode`, `addressLine1/2`, `latitude`/`longitude`); on submit it flattens to the API's snake_case `IListingPayload`. Type/property/furnishing use big icon tiles (`OptionCards`); bedrooms/bathrooms/parking use `components/ui/stepper`; amenities are toggle chips; country is dropped (defaults `PH`). The location UX lives in `features/listings/LocationPicker.tsx`: a Google Places address autocomplete (Base UI `Autocomplete` fed by `usePlaceAutocomplete`) that auto-fills the address fields, plus a draggable `AdvancedMarker` on a `<Map>` that sets the exact `latitude`/`longitude`. Base UI's `Autocomplete` calls `onChange(value)` (single arg), not `(event, value)`. In tests, mock `@/features/listings/LocationPicker` so the Google SDK isn't loaded in jsdom.
